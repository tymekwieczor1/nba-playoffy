...
# --- KONFIGURACJA ---
# Ustawienie daty startu (używamy daty z kontekstu: kwiecień 2026)
...
# CSS dla estetyki drabinki i białego tła pod logami
st.markdown("""
    <style>
...
# KROK 3: Przywrócenie stałego ładowania bazy danych
...
with tab1:
    ...
    if user:
        ...
        if saved_pin == "" or saved_pin == "nan":
            st.warning(f"Cześć {user}! Nie masz jeszcze ustalonego PINu.")
            new_pin = st.text_input("Ustal swój 6-cyfrowy PIN:", type="password", key="setup_pin")
            ...
            if pin_input == saved_pin:
                ...
                if st.button("Zapisz moje typy", disabled=is_locked, use_container_width=True):
                    ...
                    save_data(st.session_state.db)
                    st.success("✅ Typy zapisane pomyślnie!")
            elif pin_input != "":
                st.error("Błędny PIN!")
    ...
    if not st.session_state.db:
        ...
        for p in PLAYERS:
            ...
            for match in SERIES:
                ...
                # Ukrywamy typy innych przed startem
    ...
    def bracket_card(t1, seed1, t2, seed2):
        ...
        <div class="match-box">
            ...
            <div style="text-align: center; margin: 5px 0;">
                <span style="color: #666; font-size: 0.8em; background: #222; padding: 2px 10px; border-radius: 10px;">Twoje: {my_pick}</span>
            </div>
            <div style="display: flex; align-items: center;">
