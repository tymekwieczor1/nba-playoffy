...
with tab6:
    ...
    if admin_auth == ADMIN_PIN:
        ...
        for k in ALL_KEYS:
            t1, t2 = BRACKET[k][0], BRACKET[k][1]
            if t1 == "TBD" or t2 == "TBD":
                new_results[k], new_odds[f"{k}_T1"], new_odds[f"{k}_T2"], new_times[k] = actual_res_db.get(k, "W toku"), clean_odd(odds_db.get(f"{k}_T1", "")), clean_odd(odds_db.get(f"{k}_T2", "")), times_db.get(k, DEFAULT_DEADLINE)
                continue
